...
Gear.prototype.act = function(step) {
...
Player.prototype.moveY = function(step, level, keys, type) {
  this.speed.y += step * gravity;
  var motion = new Vector(0, this.speed.y * step);
  var newPos = this.pos.plus(motion);
  var obstacle = level.obstacleAt(newPos, this.size);
  if (keys.up && ladder) this.speed.y -= ladderSpeed;
  ...
  if (obstacle == "lava")
  {
	this.pos = new Vector (5, 8);  
  }
  
  
  if (keys.up && obstacle == "ladder"){
	  gravity = 0;
	  ladder = true;
  }
  gravity = 30;
...
Level.prototype.playerTouched = function(type, actor, other) {	
	if(type == 'coin') {
